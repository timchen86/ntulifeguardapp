...
class ntulgUser(models.Model):
    stage_no = models.CharField(verbose_name=u"救生班期數(stage no.)", validators=[RegexValidator(r'[0-9]+')], max_length=6, help_text=u"非水協期數") #, help_text="cm")
    cap_no = models.CharField(verbose_name=u"泳帽號碼(cap no.)", validators=[RegexValidator(r'[0-9]+')],blank=True, null=True, max_length=4, help_text=u"非必填(optional)")#, help_text="cm")
    name_cht = models.CharField(verbose_name=u"中文姓名(chinese name)", max_length=INPUT_MAX) 
    name_eng = models.CharField(verbose_name=u"英文姓名(english name)", max_length=INPUT_MAX, help_text=u"格式:LI, DAFA(李,大發)(姓放前面,然後逗點,然後名) <a href=\"http://www.englishname.org/\" target=\"_blank\">姓名英譯查詢</a>", validators=[RegexValidator(r'^[a-zA-Z\- ]+,[a-zA-Z\- ]+$')]) 
    birthday = models.DateField(verbose_name=u"生日(birthday)",max_length=10, help_text="格式:西元年-月-日,例如:1980-2-29(format: year-month-day)")
    ...
    tel = models.CharField(verbose_name=u"電話(telephone no.)", blank=True, null=True,max_length=INPUT_MAX, help_text=u"格式:(xx)xxxxxxxxx, 非必填(optional)", validators=[RegexValidator(r'^\(?[0-9]*\)\s*[0-9]*$'),MinLengthValidator(8),MaxLengthValidator(18)]) 
    ...
    sex = models.CharField(verbose_name="身理性別(biological sex)", choices=(
        ("male",u"男(male)"),
        ("female",u"女(female)")
        ),max_length=INPUT_MAX, help_text=u"非必填(optional)")
    occupation = models.CharField(verbose_name=u"職業(occupation)", max_length=INPUT_MAX)
    subordinate_units = models.CharField(verbose_name=u"所屬單位(subordinate units)", max_length=INPUT_MAX)
    ...
    tshirt_size = models.CharField(verbose_name=u"t-shirt尺寸(size)",blank=True,null=True, max_length=INPUT_MAX, default=None, choices=((None,u"請選擇"),("S","S"),("M","M"),("L","L"),("XL","XL"),("2XL","2XL"),("3XL","3XL")), help_text=u"<a href=\"http://www.mit-clothes.com.tw/info/size.html\" target=\"_blank\">尺寸表(size chart)</a>")
    ptt_id = models.CharField(verbose_name=u"PTT ID",max_length=INPUT_MAX, blank=True, null=True,help_text=u"非必填(optional)")
    ptt2_id = models.CharField(verbose_name=u"PTT2 ID",max_length=INPUT_MAX,blank=True, null=True, help_text=u"非必填(optional)")
    ...
    height = models.CharField(verbose_name=u"身高(height)", max_length=5, help_text=u"cm, 非必填(optional)", validators=[RegexValidator(r'[0-9]+')],null=True,blank=True)
    weight = models.CharField(verbose_name=u"體重(weight)", max_length=5, help_text=u"kg, 非必填(optional)", validators=[RegexValidator(r'[0-9]+')],null=True,blank=True)
